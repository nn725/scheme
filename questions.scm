...
(define (list-partitions total max-pieces max-value)
    (define (make-partitions lst total pieces value)
        (cond ((= total 0) (list lst))
            ((< total 0) nil)
            ((<= value 0) nil)
            ((equal? pieces 0) nil)
            (else (append (make-partitions (append lst (list value)) (- total value) (- pieces 1) value)
            (make-partitions lst total pieces (- value 1))))))
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (cons (cons 'lambda (cons (apply-to-all analyze (car lst)) (apply-to-all analyze body))) (apply-to-all analyze (cadr lst)))
           ))
        (else

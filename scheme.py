...
def make_list(lst):
    """Takes scheme list and turns into a python list """
    if lst == nil:
        return []
    return [lst.first] + make_list(lst.second)
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('bad call_frame args')
        frame = Frame(self)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        newVals = Pair(target.second, vals.second)
        env.define(target.first, do_lambda_form(newVals, env))
        return target.first
    else:
        raise SchemeError("bad argument to define")
...
def do_let_form(vals, env):
    ...
    lst = make_list(bindings)
    names, values = [], []
    for pair in lst:
        if len(pair) != 2 or not scheme_symbolp(pair[0]):
            raise SchemeError('bad binding')
        names = names + [pair[0]]
        values = values + [scheme_eval(pair[1], env)]
...
def do_and_form(vals, env):
    ...
    if vals.second == nil:
        return vals.first
    boolean = scheme_false(scheme_eval(vals.first, env))
    if boolean:
        return False
...
def do_or_form(vals, env):
    ...
    if vals == nil:
        return False
    x = scheme_eval(vals.first, env)
    boolean = scheme_false(x)
    if not boolean:
        return quote(x)
    if vals.second == nil:
        return vals.first
...
def check_formals(formals):
    ...
    """
    def formal_helper(lst, past):
        if lst == nil:
            return
        elif not scheme_symbolp(lst.first) or lst.first in past:
            raise SchemeError('bad formals')
        past.append(lst.first)
        formal_helper(lst.second, past)
    formal_helper(formals, [])
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            args = rest.map(lambda operand: scheme_eval(operand, env))
            if isinstance(procedure, PrimitiveProcedure):
                return apply_primitive(primitive, args, env)
            elif isinstance(procedure, LambdaProcedure):
